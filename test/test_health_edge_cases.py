...
from pypi_librarian.health import _ISO_PREFIX, _RECENT_PY_RE, _parse_date, score_project
...
class TestHealthEdgeCases:
    ...
    def test_requires_python_recent_logic(self, rp, expected_score_component):
        ...
        if expected_score_component > 0:
            ...
            assert result.score < 1.0
            assert any(
                "older Python" in n or "No requires_python" in n for n in result.notes
            )
    ...
    def test_score_project_all_notes_on_worst(self):
        info = _make_info(
            summary="", classifiers=[], license="", requires_python=">=2.7", yanked=True
        )
        result = score_project(info)
